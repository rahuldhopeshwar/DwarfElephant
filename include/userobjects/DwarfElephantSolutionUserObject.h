#ifndef DWARFELEPHANTSOLUTIONUSEROBJECT_H
#define DWARFELEPHANTSOLUTIONUSEROBJECT_H
...
// Forward declarations
class DwarfElephantSolutionUserObject;

template <>
InputParameters validParams<DwarfElephantSolutionUserObject>();

/**
 * User object that reads an existing solution from an input file and
 * uses it in the current simulation.
 */
class DwarfElephantSolutionUserObject : public GeneralUserObject
{
public:
  DwarfElephantSolutionUserObject(const InputParameters & parameters);
  virtual ~DwarfElephantSolutionUserObject(); // empty dtor required for unique_ptr with forward declarations
...
#endif // DWARFELEPHANTSOLUTIONUSEROBJECT_H
