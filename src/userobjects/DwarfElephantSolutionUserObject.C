...
#include "DwarfElephantSolutionUserObject.h"
...
registerMooseObject("DwarfElephantApp", DwarfElephantSolutionUserObject);

template <>
InputParameters
validParams<DwarfElephantSolutionUserObject>()
{
  ...
  params.addParam<MultiMooseEnum>(
      ...
      "form p = R1*(R0*x*m - t)/s.  Then the values provided by the DwarfElephantSolutionUserObject at point x "
...
Threads::spin_mutex DwarfElephantSolutionUserObject::_solution_user_object_mutex;

DwarfElephantSolutionUserObject::DwarfElephantSolutionUserObject(const InputParameters & parameters)
...
DwarfElephantSolutionUserObject::~DwarfElephantSolutionUserObject() {}

void
DwarfElephantSolutionUserObject::readXda()
{
...
void
DwarfElephantSolutionUserObject::readExodusII()
{
  // Define a default system name
  if (_system_name == "")
    _system_name = "DwarfElephantSolutionUserObjectSystem";
  ...
  if (num_exo_times == 0)
    mooseError("In DwarfElephantSolutionUserObject, exodus file contains no timesteps.");
  ...
  else
  {
    if (_exodus_time_index > num_exo_times)
      mooseError("In DwarfElephantSolutionUserObject, timestep = ",
                 _exodus_time_index,
                 ", but there are only ",
                 num_exo_times,
                 " time steps.");
...
Real
DwarfElephantSolutionUserObject::directValue(const Node * node, const std::string & var_name) const
{
...
Real
DwarfElephantSolutionUserObject::directValue(const Elem * elem, const std::string & var_name) const
{
  // Get the libmesh variable and system numbers
  unsigned int var_num = _system->variable_number(var_name);
  unsigned int sys_num = _system->number();
...
void
DwarfElephantSolutionUserObject::initialize()
{
}

void
DwarfElephantSolutionUserObject::finalize()
{
}

void
DwarfElephantSolutionUserObject::timestepSetup()
{
...
void
DwarfElephantSolutionUserObject::execute()
{
}

void
DwarfElephantSolutionUserObject::initialSetup()
{
  ...
  // Several aspects of DwarfElephantSolutionUserObject won't work with a DistributedMesh:
  ...
  // Produce an error for an unknown file type
    ...
    mooseError("In DwarfElephantSolutionUserObject, invalid file type (only .xda, .xdr, and .e supported)");
...
MooseEnum
DwarfElephantSolutionUserObject::getSolutionFileType()
{
  return _file_type;
}

void
DwarfElephantSolutionUserObject::updateExodusTimeInterpolation(Real time)
{
...
bool
DwarfElephantSolutionUserObject::updateExodusBracketingTimeIndices(Real time)
{
  if (_file_type != 1)
    mooseError(
        "In DwarfElephantSolutionUserObject, getTimeInterpolationData only applicable for exodusII file type");
...
unsigned int
DwarfElephantSolutionUserObject::getLocalVarIndex(const std::string & var_name) const
{
  ...
  if (it == _local_variable_index.end())
    mooseError("Value requested for nonexistent variable '",
               ...
               "' in the '",
               name(),
               "' DwarfElephantSolutionUserObject");
  return it->second;
}

Real
DwarfElephantSolutionUserObject::pointValueWrapper(Real t,
                                      const Point & p,
                                      ...
                                      const MooseEnum & weighting_type) const
{
  ...
  mooseError(
      "DwarfElephantSolutionUserObject::pointValueWrapper reaches line that it should not be able to reach.");
  return 0.0;
}

Real
DwarfElephantSolutionUserObject::pointValue(Real t, const Point & p, const std::string & var_name) const
{
...
Real
DwarfElephantSolutionUserObject::pointValue(Real libmesh_dbg_var(t),
                               const Point & p,
...
DwarfElephantSolutionUserObject::discontinuousPointValue(Real t,
                                            const Point & p,
...
std::map<const Elem *, Real>
DwarfElephantSolutionUserObject::discontinuousPointValue(Real libmesh_dbg_var(t),
                                            Point pt,
                                            const unsigned int local_var_index) const
{
  // do the transformations
  ...
  if (_file_type == 1 && _interpolate_times)
  {
    ...
    if (map.size() != map2.size())
      mooseError("In DwarfElephantSolutionUserObject::discontinuousPointValue map and map2 have different size");
    ...
    for (auto & k : map)
    {
      if (map2.find(k.first) == map2.end())
        mooseError(
            "In DwarfElephantSolutionUserObject::discontinuousPointValue map and map2 have differing keys");
      Real val = k.second;
...
RealGradient
DwarfElephantSolutionUserObject::pointValueGradientWrapper(Real t,
                                              const Point & p,
                                              const std::string & var_name,
                                              const MooseEnum & weighting_type) const
{
  ...
  mooseError("DwarfElephantSolutionUserObject::pointValueGradientWrapper reaches line that it should not be "
             "able to reach.");
  return RealGradient(0.0, 0.0, 0.0);
}

RealGradient
DwarfElephantSolutionUserObject::pointValueGradient(Real t, const Point & p, const std::string & var_name) const
{
  const unsigned int local_var_index = getLocalVarIndex(var_name);
...
RealGradient
DwarfElephantSolutionUserObject::pointValueGradient(Real libmesh_dbg_var(t),
                                       Point pt,
                                       const unsigned int local_var_index) const
{
  // do the transformations
...
DwarfElephantSolutionUserObject::discontinuousPointValueGradient(Real t,
                                                    const Point & p,
                                                    const std::string & var_name) const
{
  const unsigned int local_var_index = getLocalVarIndex(var_name);
...
std::map<const Elem *, RealGradient>
DwarfElephantSolutionUserObject::discontinuousPointValueGradient(Real libmesh_dbg_var(t),
                                                    Point pt,
                                                    const unsigned int local_var_index) const
{
  // do the transformations
...
Real
DwarfElephantSolutionUserObject::directValue(dof_id_type dof_index) const
{
...
Real
DwarfElephantSolutionUserObject::evalMeshFunction(const Point & p,
...
std::map<const Elem *, Real>
DwarfElephantSolutionUserObject::evalMultiValuedMeshFunction(const Point & p,
                                                const unsigned int local_var_index,
                                                unsigned int func_num) const
{
  // Storage for mesh function output
  ...
  for (auto & k : temporary_output)
  {
    mooseAssert(k.second.size() > local_var_index,
                "In DwarfElephantSolutionUserObject::evalMultiValuedMeshFunction variable with local_var_index "
                    << local_var_index
                    << " does not exist");
...
RealGradient
DwarfElephantSolutionUserObject::evalMeshFunctionGradient(const Point & p,
                                             const unsigned int local_var_index,
                                             unsigned int func_num) const
{
  // Storage for mesh function output
...
std::map<const Elem *, RealGradient>
DwarfElephantSolutionUserObject::evalMultiValuedMeshFunctionGradient(const Point & p,
                                                        const unsigned int local_var_index,
                                                        unsigned int func_num) const
{
  // Storage for mesh function output
...
DwarfElephantSolutionUserObject::variableNames() const
{
  return _system_variables;
}

bool
DwarfElephantSolutionUserObject::isVariableNodal(const std::string & var_name) const
{
