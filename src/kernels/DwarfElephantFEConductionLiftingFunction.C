...
///-------------------------------CONSTRUCTOR-------------------------------
DwarfElephantFEConductionLiftingFunction::DwarfElephantFEConductionLiftingFunction(const InputParameters & parameters) :
...
Real
DwarfElephantFEConductionLiftingFunction::computeQpResidual()
{
  return _lambda[_qp] *   (_grad_test[_i][_qp]*(_grad_u[_qp]-_lifting_function->gradient(_fe_problem.time(),_qp)));
}

Real
DwarfElephantFEConductionLiftingFunction::computeQpJacobian()
{
